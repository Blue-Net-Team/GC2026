...
- 默认端口：`8080`
- 绑定网卡：所有可用网卡（`config.yaml` 中 `system.udp_interface` 为空字符串）
...
- 发送内容：摄像头原始画面。
- 默认端口：`4444`
- 默认网卡：从 `config.yaml` 的 `system.udp_interface` 读取（空字符串表示所有网卡）
...
```python
# Linux 发送端：网卡从 config.yaml 的 system.udp_interface 读取（空字符串表示所有网卡）
...
- 发送端网卡由 `config.yaml` 的 `system.udp_interface` 决定，不再硬编码 `eth0`。
...
按以下顺序逐层排查（每一步都对应图传链路的一环，跳过任何一步都可能误判）：

1. **服务端真的起来了吗**：板端日志应出现 `UDP 服务已启动 (监听所有网卡)`。没有 → `uv run main` 根本没跑起来，先解决启动问题；
2. **握手包到了吗**：板端日志应出现 `UDP 客户端已连接: <PC的IP>`。没有 → 客户端的 `connect` 包没到达服务端，继续往下查网络；
3. **网络通不通**：PC 上 `ping <板子IP>`；不通 → 检查是否同网段、网线/WiFi、板子 IP 是否真的是 OLED 上显示的那个；
4. **防火墙**：PC 端防火墙放行 UDP `8080`（方式 A）/ `4444`（方式 B）；板端确认没有 iptables 规则拦截；
5. **手动验证握手**：PC 上执行 `nc -u <板子IP> 8080` 后输入 `connect` 回车，观察板端日志是否出现「UDP 客户端已连接」。能连上说明问题在客户端软件（IP/端口填错），连不上说明问题在网络层；
6. **客户端地址填对了吗**：桌面应用里填的是**板子的 IP**（OLED 上的 `Server IP`），端口默认 `8080`。方式 B 的 Windows 接收端要改 `img_trans.py` 里硬编码的 IP；
7. **`udp_target_ip` 干扰**：如果 `config.yaml` 里预配置了 `udp_target_ip` 指向别的机器，服务端会优先往那台机器发。清空该字段或改为本机 IP 后再试。
